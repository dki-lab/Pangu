...
@Tokenizer.register("my_pretrained_transformer")
class PretrainedTransformerTokenizer(Tokenizer):
    ...
    def convert_token_to_id(self, token):
        ...
        return self._tokenizer.vocab.get(token, self._tokenizer.vocab.get(self._tokenizer.unk_token))
...
def _guess_start_and_end_token_defaults(model_name: str) -> Tuple[List[str], List[str]]:
    if 'roberta' in model_name:
        return (['<s>'], ['</s>'])
    elif 'bert' in model_name:   # deberta also follows this
        return (['[CLS]'], ['[SEP]'])
